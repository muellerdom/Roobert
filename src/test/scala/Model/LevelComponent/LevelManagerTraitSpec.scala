...
class LevelManagerTraitSpec extends AnyFlatSpec with Matchers {
  ...
  "LevelManagerTrait" should "load a level" in {
    val levelManager = new LevelManagerTrait {
      ...
      override def getAvailableLevels: List[String] = List(levelName)
      override def getCurrentLevel: Option[LevelConfig] = Some(LevelConfig(levelName, "", "", 0, 0, Coordinate(0, 0), Goal(0, 0), Objects(Nil, Nil)))
    }
